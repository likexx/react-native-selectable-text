...
NSString *const RNSELECTABLETEXT_CUSTOM_SELECTOR = @"_CUSTOM_SELECTOR_";
...
-(void) _handleGesture
{
    ...
    for(NSString *menuItemName in self.menuItems) {
        NSString *sel = [NSString stringWithFormat:@"%@%@", RNSELECTABLETEXT_CUSTOM_SELECTOR, menuItemName];
...
- (void)forwardInvocation:(NSInvocation *)invocation
{
    ...
    NSRange match = [sel rangeOfString:RNSELECTABLETEXT_CUSTOM_SELECTOR];
    if (match.location == 0) {
